...
/**
 * Busca dados com re-tentativa caso venha vazio
 */
async function fetchTableWithRetry(tableName, retries = 3) {
    for (let i = 0; i < retries; i++) {
        try {
            const data = await grist.docApi.fetchTable(tableName);
            const records = Array.isArray(data) ? data : (data.records || []);
            if (records.length > 0) return records;
            logDebug(`Tentativa ${i+1} para ${tableName}: 0 registros encontrados.`);
        } catch (e) {
            logDebug(`Erro tentativa ${i+1} para ${tableName}: ${e.message}`);
        }
...
async function updateDashboard() {
    ...
    allRecords.forEach(r => {
        ...
        for (let i = 0; i < Math.min(faltantes, 6); i++) projectionData[i] += valParcela;
    });
...
grist.onRecords(async (records) => {
    ...
    try {
        ...
        // Aguarda um pouco para o Grist processar as permissões
        await new Promise(r => setTimeout(r, 500));

        // Busca tabelas auxiliares com re-tentativa
        bancoData = await fetchTableWithRetry('Banco');
        cartoesData = await fetchTableWithRetry('Cartoes');

        logDebug(`Carga Finalizada: Bancos=${bancoData.length}, Cartões=${cartoesData.length}`);
        
        if (bancoData.length > 0) {
            logDebug("Campos da Tabela Banco:", Object.keys(bancoData[0].fields || bancoData[0]));
        }
